...
let
  ...
  inherit (lib)
    mkOption mkEnableOption mkIf types optional toLower listToAttrs
    nameValuePair;
...
in {
  options.programs.lutris = {
    ...
    extraPkgs = mkOption {
      ...
      default =
        (with pkgs; [ gamemode mangohud winetricks gamescope libstrangle ]);
    };
    ...
    runners =
      mkOption { type = types.listOf (types.either types.package runnerType); };
  ...
  config = mkIf cfg.enable {
    ...
    home.packages = let
      lutris-override = {
        ...
        extraPkgs = pkgs:
          cfg.extraPkgs
          ++ optional (cfg.steamPackage != null) cfg.steamPackage;
      };
    ...
    xdg.dataFile = listToAttrs (map (wine-pkg:
      let name = toLower wine-pkg.name;
    ...
    in builtins.foldl' (acc: runner:
      acc // (if (types.package.check runner) then
        (runnerToConfig (pkgToRunner runner))
      else
        runnerToConfig runner)) { } cfg.runners;
  };
}
